...
export async function mockTimeBatchInsert(
    {model,field,total,unitNum,unit,format,}: {model:any;field:any;total:any;unitNum: number;unit: dayjs.ManipulateType;format:any;}) {
...
export const loadAllModel = () => {
    const modelDir = `${process.cwd()}/${PROJECT_ENV === PROJECT_ENV_ENUM.prod ? 'dist' : 'src'
        }/model`;
