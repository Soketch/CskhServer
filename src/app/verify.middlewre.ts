...
// 允许频繁请求的路径白名单
const frequentlyWhiteList: string | string[] = [];

export const apiBeforeVerify = async (ctx: ParameterizedContext, next: any) => {
...
export async function handleVerifyAuth({ ctx, shouldAuthArr }: { ctx: any; shouldAuthArr: string[] }) {
...
export const apiVerifyAuth = (shouldAuthArr: string[]) => {
    return async (ctx: ParameterizedContext, next : any) => {
