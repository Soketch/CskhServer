...
import { IList, IAuth } from "@/interface";
...
const getArrayDifference = <T>(array1: T[], array2: T[]): T[] => _.difference(array1, array2);
...
class AuthController {
    ...
    async commonGetAllChildAuth(id:number) {
        const allAuth: any = [];
        const queue: any = [];
    ...
    /** 获取我的权限 */
    getMyAuth = async (ctx: ParameterizedContext, next:any) => {
    ...
    /** 获取某个用户的权限 */
    getUserAuth = async (ctx: ParameterizedContext, next:any) => {
    ...
    // 权限列表（不分页）
    async getAllList(ctx: ParameterizedContext, next:any) {
    ...
    // 获取所有权限（树型）
    async getTreeAuth(ctx: ParameterizedContext, next:any) {
    ...
    // 获取除了父级以外的所有权限（树型）
    async getTreeChildAuth(ctx: ParameterizedContext, next:any) {
    ...
    // 获取该权限的子权限（只找一层）
    getChildAuth = async (ctx: ParameterizedContext, next:any) => {
        const id = +ctx.params.id;
    ...
    // 获取该权限的子权限（递归查找所有）
    getAllChildAuth = async (ctx: ParameterizedContext, next:any) => {
        const id = +ctx.params.id;
        const isExist = await authService.isExist([id]);
        if (!isExist) {
            throw new CustomError(
    ...
    async create(ctx: ParameterizedContext, next:any) {
        const body: IAuth = ctx.request.body;
    ...
    // 删除权限
    delete = async (ctx: ParameterizedContext, next:any) => {
        const id = +ctx.params.id;
        if (PROJECT_ENV === PROJECT_ENV_ENUM.beta) {
            const role: any = await authService.find(id);
            if (role.type === 1) {
    ...
    // 批量新增子权限
    batchAddChildAuths = async (ctx: ParameterizedContext, next:any) => {
        ...
        const result2: number[] = result1.map((v:any) => v.p_id);
    ...
    // 批量删除子权限
    batchDeleteChildAuths = async (ctx: ParameterizedContext, next: any) => {
        const { id, c_auths }: IAuth = ctx.request.body;
        if (!id) {
            throw new CustomError(
                `id不能为空！`,
        ...
        const all_child_auths_id = all_child_auths.map((v: any) => v.id);
